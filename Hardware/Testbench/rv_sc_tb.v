...
module tb_rv_single_axi();
        ...
        if (!rst_n) begin
            ...
            case (i_state)
                0: if (m_inst_axi_arvalid) begin
                       m_inst_axi_arready <= 1; //Receive address
                       i_state <= 1;
                   end
                1: begin
                       m_inst_axi_arready <= 0;
                ...
                2: if (m_inst_axi_rready) begin
                       m_inst_axi_rvalid <= 0; //Finish handshake
                       i_state <= 0;
                   end
            endcase
        end
        ...
        if (!rst_n) begin
            ...
            case (dw_state)
                0: if (m_data_axi_awvalid) begin
                       ...
                       waddr_latch <= m_data_axi_awaddr; // Block address
                       dw_state <= 1;
                   end
                1: begin
                ...
                2: begin
                       m_data_axi_wready <= 0;
                       m_data_axi_bvalid <= 1;
                       m_data_axi_bresp  <= 2'b00; //Call back OK
                       dw_state <= 3;
                   end
                3: if (m_data_axi_bready) begin
                       m_data_axi_bvalid <= 0; //Finish handshake
                       dw_state <= 0;
                   end
            endcase
        end
    ...
    initial begin
        ...
        //Wait for done to high
       begin : WAIT_BLOCK
            fork
                begin
                    wait(done == 1'b1);
                    $display("\n[SUCCESS] CPU asserts DONE flag at time %0t", $time);
                    disable WAIT_BLOCK; //Break loop
                end
                begin
                    #500000; //Time out force end
